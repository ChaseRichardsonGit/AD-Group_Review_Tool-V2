...
function New-HTMLReport {
    ...
    try {
        ...
        # Calculate OU activity
        $ouActivity = $Groups | Group-Object OU | Select-Object @{
            Name = 'OU'
            Expression = { $_.Name }
        }, @{
            Name = 'Activity'
            Expression = { ($_.Group | Measure-Object TotalMembers -Sum).Sum }
        }, @{
            Name = 'AvgHealth'
            Expression = { ($_.Group | Measure-Object HealthScore -Average).Average }
        }, @{
            Name = 'LastModified'
            Expression = { ($_.Group | Sort-Object Created -Descending | Select-Object -First 1).Created }
        }, @{
            Name = 'CommonType'
            Expression = { ($_.Group | Group-Object Category | Sort-Object Count -Descending | Select-Object -First 1).Name }
        }

        $mostActiveOU = $ouActivity | Sort-Object Activity -Descending | Select-Object -First 1
        $leastActiveOU = $ouActivity | Where-Object { $_.Activity -gt 0 } | Sort-Object Activity | Select-Object -First 1
...
<head>
    ...
    <script>
        function filterGroups(filterType, value, element) {
            // Remove active class from all elements in the same category
            document.querySelectorAll('.active-filter').forEach(el => {
                if (el !== element && el.getAttribute('data-filter-type') === filterType) {
            ...
            if (element) {
                const wasActive = element.classList.contains('active-filter');
                element.classList.toggle('active-filter');
                if (wasActive) {
                    // If it was active and we're removing the filter, show all rows
                    showAllRows();
                    return;
                }
            }
                ...
                switch(filterType) {
                    case 'health':
                        ...
                        if (value === 'all') {
                            ...
                            show = healthScore >= min && healthScore <= max;
                        }
                        break;
                    case 'issue':
                        switch(value) {
                            case 'empty':
                                show = row.querySelector('.member-stats').textContent.includes('Total: 0');
                                break;
                            case 'no-manager':
                                show = row.querySelector('.manager-info')?.textContent.includes('No manager assigned') ?? false;
                                break;
                            case 'no-description':
                                show = row.querySelector('.description').textContent.trim() === '';
                                break;
                            case 'nested':
                                show = row.querySelector('.nested-warning') !== null;
                                break;
                        }
                        break;
                }
        ...
        function filterByOU(ou) {
            // Remove active class from all rows
            document.querySelectorAll('tr.active-filter').forEach(el => {
                el.classList.remove('active-filter');
            });

            const clickedRow = Array.from(document.querySelectorAll('tr[data-filter-type="ou"]'))
                .find(row => row.children[0].textContent === ou);

            if (clickedRow.classList.contains('active-filter')) {
                showAllRows();
                return;
            }
            ...
            const rows = document.querySelectorAll('table tbody tr');
            rows.forEach(row => {
                const ouCell = row.querySelector('.group-details span:last-child');
                if (ouCell) {
                    row.style.display = ouCell.textContent === ou ? '' : 'none';
                }
            });
        }
    </script>
...
<body>
    <div class="container">
        <div class="title-container">
            <h1>Active Directory Group Health Report</h1>
            <div class="subtitle">Comprehensive Analysis and Optimization Report</div>
        </div>

        <div class="interactive-note">
            <strong>📌 Interactive Report:</strong> Click on health categories, issues, or statistics to filter the table below. Click again to remove the filter.
        </div>
        ...
        <div class="dashboard">
            ...
            <div class="card">
                ...
                <div class="stat">
                    <span class="stat-label">Active/Disabled Ratio</span>
                    <span class="stat-value">$([Math]::Round($activeMembers / ($disabledMembers + $activeMembers) * 100, 1))%</span>
                    <div class="stat-icon-container">
                        <svg class="stat-icon" viewBox="0 0 24 24">
                ...
                <div class="stat">
                    <span class="stat-label">Most Active OU</span>
                    <span class="stat-value">$($mostActiveOU.OU -replace '^.*?([^,]+).*$', '$1')</span>
                    <div class="stat-icon-container">
                        <svg class="stat-icon" viewBox="0 0 24 24">
                ...
                <div class="stat">
                    <span class="stat-label">Least Active OU</span>
                    <span class="stat-value">$($leastActiveOU.OU -replace '^.*?([^,]+).*$', '$1')</span>
                    <div class="stat-icon-container">
                        <svg class="stat-icon" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm0 12H4V8h16v10z"/>
                        </svg>
                ...
                <div class="stat">
                    <span class="stat-label">Oldest Group</span>
                    <span class="stat-value">$($oldestGroup.Created.ToString('yyyy-MM-dd'))</span>
                    <div class="stat-icon-container">
                        <svg class="stat-icon" viewBox="0 0 24 24">
                ...
                <div class="stat">
                    <span class="stat-label">Newest Group</span>
                    <span class="stat-value">$($newestGroup.Created.ToString('yyyy-MM-dd'))</span>
                    <div class="stat-icon-container">
                        <svg class="stat-icon" viewBox="0 0 24 24">
                            <path fill="currentColor" d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z"/>
                        </svg>
            ...
            <div class="card">
                ...
                <div class="health-distribution">
                    <div class="health-segment neutral clickable" onclick="filterGroups('health', 'all', this)" data-filter-type="health">
                        <div>All Groups</div>
                        <strong>$totalGroups</strong>
                    </div>
                    <div class="health-segment critical clickable" onclick="filterGroups('health', '0-50', this)" data-filter-type="health">
                        <div>Critical (0-50)</div>
                        <strong>$criticalGroups</strong>
                    </div>
                    <div class="health-segment warning clickable" onclick="filterGroups('health', '51-80', this)" data-filter-type="health">
                        <div>Warning (51-80)</div>
                        <strong>$warningGroups</strong>
                    </div>
                    <div class="health-segment healthy clickable" onclick="filterGroups('health', '81-100', this)" data-filter-type="health">
                        <div>Healthy (81-100)</div>
            ...
            <div class="card">
                ...
                <div class="stat clickable" onclick="filterGroups('issue', 'empty', this)" data-filter-type="issue">
                ...
                <div class="stat clickable" onclick="filterGroups('issue', 'no-manager', this)" data-filter-type="issue">
                ...
                <div class="stat clickable" onclick="filterGroups('issue', 'no-description', this)" data-filter-type="issue">
                ...
                <div class="stat clickable" onclick="filterGroups('issue', 'nested', this)" data-filter-type="issue">
            ...
            <table class="ou-stats-table">
                <thead>
                    <tr>
                        ...
                        <th colspan="3" class="no-bottom-border">Additional Info</th>
                    </tr>
                    <tr>
                        ...
                        <th class="top-border">Avg Health</th>
                        <th class="top-border">Last Modified</th>
                        <th class="top-border">Common Type</th>
                    </tr>
                </thead>
                <tbody>
                    $(foreach ($stat in $ouStats) {
                        $ouStat = $ouActivity | Where-Object { $_.OU -eq $stat.OU } | Select-Object -First 1
                        ...
                        $healthClass = if ($ouStat -and $ouStat.AvgHealth -le 50) { 'critical-text' } elseif ($ouStat -and $ouStat.AvgHealth -le 80) { 'warning-text' } else { '' }
                        $avgHealth = if ($ouStat) { [Math]::Round($ouStat.AvgHealth, 1) } else { 0 }
                        $lastModified = if ($ouStat -and $ouStat.LastModified) { $ouStat.LastModified.ToString('yyyy-MM-dd') } else { 'N/A' }
                        $commonType = if ($ouStat -and $ouStat.CommonType) { $ouStat.CommonType } else { 'N/A' }
                        
                        @"
                        <tr class="clickable" onclick="filterByOU('$($stat.OU)')" data-filter-type="ou">
                            <td>$($stat.OU)</td>
                            ...
                            <td class="health-cell $healthClass">$avgHealth%</td>
                            <td class="date-cell">$lastModified</td>
                            <td class="type-cell">$commonType</td>
                        </tr>
"@
                    })
                </tbody>
...
try {
    ...
    # Function to check if any OUs are selected and update button state
    function Update-GenerateButtonState {
        ...
        $GenerateButton.IsEnabled = $anySelected
    }
    ...
    $OUList.ItemContainerGenerator.StatusChanged += {
        if ($OUList.ItemContainerGenerator.Status -eq [System.Windows.Controls.Primitives.GeneratorStatus]::ContainersGenerated) {
            $OUList.Items | ForEach-Object {
                $container = $OUList.ItemContainerGenerator.ContainerFromItem($_)
                if ($container) {
                    $checkbox = $container.ContentTemplate.FindName("ouCheckBox", $container)
                    ...
                    if ($checkbox) {
                        $checkbox.Add_Checked({ Update-GenerateButtonState })
                        $checkbox.Add_Unchecked({ Update-GenerateButtonState })
                    }
                }
            }
        }
