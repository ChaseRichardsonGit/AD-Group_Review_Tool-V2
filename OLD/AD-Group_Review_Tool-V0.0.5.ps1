...
function New-HTMLReport {
    ...
    try {
        ...
        $ouStats = $script:OUStats.GetEnumerator() | ForEach-Object {
            # Clean up OU name by removing OU= prefixes and DC components
            $ouName = $_.Key -replace '^OU=|,OU=|,DC=.*$',''
            $stats = $_.Value
                    ...
                    <tbody>
                        $(foreach ($stat in $ouStats) {
                            ...
                            <tr>
                                ...
                                <td class="text-center">$($stat.EnabledMembers)</td>
                                <td class="text-center">$($stat.DisabledMembers)</td>
