...
function Get-GroupHealth {
    ...
    if ([string]::IsNullOrWhiteSpace($Group.Description)) {
        $health.Issues = @($health.Issues) + @("Missing description")
        $health.Score = $health.Score - 20
    }
    
    # Check for missing manager
    if ([string]::IsNullOrWhiteSpace($Group.Manager)) {
        $health.Issues = @($health.Issues) + @("No manager assigned")
        $health.Score = $health.Score - 20
    }
    
    # Check member count
    if ($Group.TotalMembers -eq 0) {
        $health.Issues = @($health.Issues) + @("Empty group")
        $health.Score = $health.Score - 30
    }
    elseif ($Group.TotalMembers -gt 1000) {
        $health.Issues = @($health.Issues) + @("Large group (>1000 members)")
    ...
    if ($age.Days -gt 365 * 2) {
        $health.Issues = @($health.Issues) + @("Group older than 2 years")
        $health.Score = $health.Score - 10
    }
    ...
    if ($Group.UserMembers -gt 0) {
        ...
        if ($disabledPercentage -gt 40) {
            $health.Issues = @($health.Issues) + @("High percentage of disabled users (>40%)")
            $health.Score = $health.Score - 30
        }
        elseif ($disabledPercentage -gt 20) {
            $health.Issues = @($health.Issues) + @("Moderate percentage of disabled users (>20%)")
...
function Get-NestedGroupMembership {
    ...
    try {
        ...
        # Use ArrayList for better performance with older DCs
        ...
        if ($group.memberOf -is [Microsoft.ActiveDirectory.Management.ADPropertyValueCollection]) {
            # Handle AD Property Value Collection
            foreach ($memberOfGroup in $group.memberOf) {
                if (-not $ProcessedGroups.Contains($memberOfGroup)) {
                    [void]$nestedGroups.Add($memberOfGroup)
                    $moreNestedGroups = Get-NestedGroupMembership -GroupDN $memberOfGroup -ProcessedGroups $ProcessedGroups
                    if ($moreNestedGroups.Count -gt 0) {
                        [void]$nestedGroups.AddRange($moreNestedGroups)
                    }
                }
            }
        } else {
            # Handle regular array or other collection types
        ...
        # Convert back to array and ensure uniqueness
        ...
        foreach($ou in $SelectedOUs) {
            ...
            foreach($group in $groups) {
                ...
                try {
                    ...
                    ${script:OUStats}[$ou].NestedGroupCount = ${script:OUStats}[$ou].NestedGroupCount + $nestingDepth
                    ...
                    if ($group.member) {
                        ...
                        ${script:OUStats}[$ou].EnabledMembers = ${script:OUStats}[$ou].EnabledMembers + $enabledMembers
                        ${script:OUStats}[$ou].DisabledMembers = ${script:OUStats}[$ou].DisabledMembers + $disabledMembers
                        ${script:OUStats}[$ou].TotalMembers = ${script:OUStats}[$ou].TotalMembers + $totalMembers
                    }
